 ...
 * "please confirm acceptance" (email-acceptance workflow). Estimates never
 * ask for a signature (non-binding) or say "this proposal".
 */
function getNextStepsText(data: InvoiceData, isEstimate = false): string {
    if (data.paymentInstructions) return data.paymentInstructions
    if (isEstimate) {
        return "This is a planning estimate — no signature is required. Reply or reach out with any questions, and we'll follow up with a firm quote or proposal once the scope is confirmed."
    }
        ...
        <Document>
            <Page size="A4" style={{ paddingTop: 40, paddingBottom: 56, fontSize: 10, fontFamily: c.font, backgroundColor: "#fff", ...bNone() }} wrap>
                ...
                <View style={{ marginHorizontal: 48, marginTop: 4, marginBottom: 26, padding: 18, backgroundColor: c.acc, ...r(8), ...bNone(), borderLeftWidth: 4, borderLeftColor: c.pri, borderLeftStyle: "solid" as any, borderTopWidth: 0, borderRightWidth: 0, borderBottomWidth: 0, borderTopColor: "transparent", borderRightColor: "transparent", borderBottomColor: "transparent", borderTopStyle: "solid" as any, borderRightStyle: "solid" as any, borderBottomStyle: "solid" as any }} wrap={false}>
                    ...
                    <Text style={{ fontSize: 10, color: c.txt, lineHeight: 1.5 }}>{getNextStepsText(data, isEstimate)}</Text>
                </View>

                {/* â”€â”€ SIGNATURE BLOCKS â”€â”€ */}
                {/* Estimates are non-binding planning documents — they should
                    never require a signature (that implies a binding acceptance,
                    which is what a Proposal/Quote is for). Always hidden for
                    estimates regardless of showSignatureFields. */}
                {!isEstimate && data.showSignatureFields !== false && (
                    <View style={{ flexDirection: "row", paddingHorizontal: 48, marginBottom: 20, ...bNone() }} wrap={false}>
                        {[
