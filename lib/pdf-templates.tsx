...
export function PaymentReceiptPDF({ receiptData }: { receiptData: PaymentReceiptData }) {
    ...
    const amountDisplay = receiptData.currency === "INR"
        ? `Rs. ${(receiptData.amount / 100).toLocaleString("en-IN", { minimumFractionDigits: 2 })}`
        : `${receiptData.currency} ${(receiptData.amount / 100).toFixed(2)}`

    const dateDisplay = (() => {
        try { return new Date(receiptData.date).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" }) }
        catch { return receiptData.date }
    })()

    const s = StyleSheet.create({
        page: { padding: 48, fontSize: 10, fontFamily: "Inter", backgroundColor: "#fff" },
        topBar: { height: 6, backgroundColor: pri, marginBottom: 0, ...bNone() },
        header: { paddingTop: 32, paddingBottom: 24, ...bBottom(1, bdr) },
        title: { fontSize: 28, color: pri, letterSpacing: 1, fontWeight: 700 },
        sub: { fontSize: 11, color: mut, marginTop: 4 },
        badge: { marginTop: 12, backgroundColor: bg, paddingHorizontal: 12, paddingVertical: 5, ...r(14), alignSelf: "flex-start" as any },
        section: { marginTop: 28 },
        row: { flexDirection: "row", justifyContent: "space-between", paddingVertical: 10, ...bBottom(1, bdr) },
        label: { fontSize: 10, color: mut },
        value: { fontSize: 10, color: txt, fontWeight: 700 },
        totalRow: { flexDirection: "row", justifyContent: "space-between", paddingTop: 16, marginTop: 4 },
        totalLabel: { fontSize: 14, color: pri, fontWeight: 700 },
        totalValue: { fontSize: 20, color: pri, fontWeight: 700 },
        footer: { position: "absolute", bottom: 32, left: 48, right: 48, ...bTop(1, bdr), paddingTop: 12 },
        footerText: { fontSize: 8, color: mut, textAlign: "center" },
    })
        ...
        <Document>
            <Page size="A4" style={s.page}>
                ...
                <View style={s.header}>
                    ...
                    <Text style={s.sub}>Clorefy — AI Document Platform</Text>
                    <View style={s.badge}>
                        <Text style={{ fontSize: 9, color: pri, fontWeight: 700 }}>PAID</Text>
                    </View>
                </View>
                ...
                <View style={s.section}>
                    <View style={s.row}>
                        ...
                        <Text style={s.value}>{dateDisplay}</Text>
                    </View>
                    <View style={s.row}>
                        <Text style={s.label}>Payment ID</Text>
                        <Text style={s.value}>{receiptData.paymentId}</Text>
                    </View>
                    <View style={s.row}>
                        <Text style={s.label}>Order ID</Text>
                        <Text style={s.value}>{receiptData.orderId}</Text>
                    </View>
                    <View style={s.row}>
                        <Text style={s.label}>Plan</Text>
                        <Text style={s.value}>{receiptData.plan} ({receiptData.billingCycle})</Text>
                    </View>
                    <View style={s.row}>
                        <Text style={s.label}>Billed To</Text>
                        <Text style={s.value}>{receiptData.userEmail || "—"}</Text>
                    </View>
                    <View style={s.totalRow}>
                        <Text style={s.totalLabel}>Amount Paid</Text>
                        <Text style={s.totalValue}>{amountDisplay}</Text>
                    </View>
                </View>

                <View style={s.footer} fixed>
                    <Text style={s.footerText}>
                        Thank you for subscribing to Clorefy. This is your official payment receipt.
