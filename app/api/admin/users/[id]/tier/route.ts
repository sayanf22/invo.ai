...
export async function PATCH(
  ...
  if (!isValidUUID(userId)) return NextResponse.json({ error: "Not found" }, { status: 404 })
  ...
  const { tier, expires_at: expiresAt, reason } = body
  ...
  try {
    ...
    console.error("[admin/tier] atomic override failed:", error)
